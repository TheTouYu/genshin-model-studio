...
浏览器里手画线条 → 自动拟合成曲线 → 拆成官方基础元件 → 实时 3D 预览 → 一键导出 .gil/.gia。

```bash
npm run web             # 本地服务器，打开 http://localhost:8787
```

操作流程：

1. 滚到 **✏️ 画线建模** 区域，在画布上画线（鼠标拖拽；移动端手指直接画）；
2. 松手自动拟合：画布上叠加显示拟合曲线（琥珀色 = 开放线，青色 = 封闭图形）；
3. 右侧 3D 预览实时更新（拖拽旋转 / 滚轮缩放 / 移动端单指拖拽、双指捏合）；
4. 调参数：**模式**（挤出 = 杆状拼线；旋转 = 母线转出碗/花瓶类旋转体）、**形状**（圆柱/方杆）、**尺寸**（杆径）、**数量**（10~300，元件数）、**模型高**（米）；
5. **颜色**（三期）：色板点色 → 新笔画自动上色；点笔画颜色圆点可改已画笔画的颜色，预览即时反映；
6. 导出：填模型名字 → **导出 .gil / .导出 .gia**；作品自动保存在本机浏览器（localStorage），可导出/导入作品 JSON 备份。

> 画布高默认 10 米（可调）；同一模型放多块时手动改 prefabId/aux ID（见输入格式文档）。
> 设计与契约：[`docs/phase2-drawing-prd.md`](docs/phase2-drawing-prd.md)。
