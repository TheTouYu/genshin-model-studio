...
| # | type | 值字段 | 内容 | 语义 |
|---|---|---|---|---|
| 0 | 1 | 11 | `{1: pos, 2: rot, 3: scale, 501: -1}` | **模型根节点 Transform**（两样本 v0 均为 (0,0,0)/1.0 → 推测为新建默认；足球当前版已移到 (-3,1,0)/0.25，随版本变化）。**本项目写入语义（2026-09-09 用户实测）**：真实尺寸 = root × 项数据（root 同时乘位置与缩放）→ `rootTransform.scale = 主模型缩放 S × 整体缩放率 K`，item `position`/`scale` 均 ÷S；网页画线导出区两个参数（`rootScale` 默认 0.1 / `overallScale` 默认 1）即此语义 |
