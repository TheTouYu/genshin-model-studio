...
- **接口侧别取证（12× 放大 view-left / view-right 的底座侧壁，请以此判定侧别，不要凭主视图小图推断）**：
  - views/diag/view-left-portzoom.png —— view-left（yaw=+1.5708 → 相机在 **+X** 侧）→ 看到 **x=+0.1520 壁**：应为 **1 个圆形耳机孔**
  - views/diag/view-right-portzoom.png —— view-right（yaw=−1.5708 → 相机在 **−X** 侧）→ 看到 **x=−0.1520 壁**：应为 **2 个矩形 USB-C 开口**
...
坐标系：+z 朝用户（前缘）。**相机方位映射（必须按此判读）**：yaw=0 → 相机在 +Z（front）；yaw=+1.5708（left 预设）→ 相机在 **+X** 侧 → 看到 x=+0.1520 壁（耳机孔）；yaw=−1.5708（right 预设）→ 相机在 **−X** 侧 → 看到 x=−0.1520 壁（双 USB-C）。USB-C 在 x=−0.1520，耳机孔在 x=+0.1520（与 §2 规格一致，已由 items.json 坐标核验）。
