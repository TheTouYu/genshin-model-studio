...
surface(mirrorRings(TORSO_R, 1), U, function (i) {
  var r = TORSO_R[Math.min(i, TORSO_R.length - 1)];
  return r.y < 0.40 ? SKIN : (r.y < 0.92 ? WHITE : SKIN); // 上衣/短裤覆盖区全白，杜绝腰腹露肤
}, 0.0012);
var jerseyRings = ringsBetween(TORSO_R, 0.56, 0.92, 10);
...
var shortsRings = ringsBetween(TORSO_R, 0.40, 0.63, 8);
surface(offsetRings(shortsRings, 0.006), U, function (i, j) { return WHITE; }, 0.0016);
window.gms.part('disc', { x: 0, y: 0.63, z: -0.02, r: 0.060, thick: 0.012, axis: 'up', color: BLUE });
// 短裤前侧 10（参考图）
window.gms.part('rod', { x1: -0.052, y1: 0.565, z: 0.078, x2: -0.052, y2: 0.505, size: 0.005, color: BLUE });
window.gms.part('disc', { x: -0.024, y: 0.535, z: 0.078, r: 0.016, thick: 0.002, axis: 'front', color: BLUE });

/* ---------- 腿（皮肤底座（下） + 白袜壳层（上，外扩 0.005） + 蓝条） ---------- */
function leg(sign) {
  var sockEnd = 0;
  while (sockEnd < LEG_R.length && LEG_R[sockEnd].y < 0.30) sockEnd++;
  var sock = mirrorRings(LEG_R.slice(0, Math.max(2, sockEnd + 1)), sign);
  surface(offsetRings(sock, 0.005), Math.round(U * 0.8), function () { return SOCK; }, 0.0014);
  var skin = mirrorRings(LEG_R.slice(sockEnd), sign);
...
function arm(side) {
  surface([
    ...
    { y: 0.86, cx: side * 0.88, rx: 0.026, ry: 0.026, cz: 0.01 },
    { y: 0.79, cx: side * 0.91, rx: 0.024, ry: 0.024, cz: 0.02 },
    { y: 0.74, cx: side * 1.00, rx: 0.022, ry: 0.022, cz: 0.03 },
    { y: 0.70, cx: side * 1.14, rx: 0.021, ry: 0.021, cz: 0.035 },
    { y: 0.58, cx: side * 1.33, rx: 0.020, ry: 0.020, cz: 0.04 },
  ], Math.round(U * 0.7), function (i) { return SKIN; }, 0.0012);
  surface(offsetRings([
    { y: 0.93, cx: side * 1.0, rx: 0.024, ry: 0.024, cz: 0.0 },
    { y: 0.88, cx: side * 0.90, rx: 0.026, ry: 0.026, cz: 0.005 },
    { y: 0.80, cx: side * 0.93, rx: 0.024, ry: 0.024, cz: 0.02 },
  ], 0.006), Math.round(U * 0.7), function () { return WHITE; }, 0.0014);
  window.gms.part('disc', { x: side * 0.93, y: 0.80, z: 0.02, r: 0.027, thick: 0.009, axis: 'up', color: BLUE });
  window.gms.part('disc', { x: side * 1.38, y: 0.56, z: 0.04, r: 0.024, thick: 0.016, axis: 'front', color: GLOVE });
}
arm(-0.105);
arm(0.105);
...
window.gms.part('sphere', { x: 0, y: 1.115, z: -0.02, r: 0.082, color: HAIR3 });
...
for (var li = 0; li < 14; li++) {
  ...
  ribbon([[sx, sy, 0.03], [sx + di * 0.012, sy - 0.06, 0.05], [sx + di * 0.008, sy - 0.13, 0.045], [sx + di * 0.016, sy - 0.14, 0.035]],
...
function horn(sd) {
  var bx = 0.05 * sd;
  ribbon([[bx, 1.08, 0], [bx + 0.016 * sd, 1.13, -0.02], [bx + 0.024 * sd, 1.19, -0.05], [bx + 0.010 * sd, 1.22, -0.08]],
    [0.016, 0.015, 0.012], HORN, headC);
  window.gms.part('cone', { x: bx + 0.010 * sd, y: 1.24, z: -0.08, r: 0.015, h: 0.045, axis: 'up', color: HORN_TIP });
}
horn(-1); horn(1);
