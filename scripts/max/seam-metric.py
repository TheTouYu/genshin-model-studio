...
"""
⚠ 使用前必读（r43 教训）：本脚本的 rows/cols 是**盲区**——r41–r43 有五次对照实验
把机身**前缘唇口**（z≈110.4 的掠射亮带）当成「触控板缝」在量，白白废掉五轮改动。
任何缝相关读数必须先跑 scripts/max/ray-pick.mjs 确认被测像素的世界坐标分类是
TP-EDGE，再喂给本脚本；分类不是 TP-EDGE 时本脚本的数字无效。
