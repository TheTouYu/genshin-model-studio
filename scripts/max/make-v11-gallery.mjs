...
const protocol = [
  ['p1-closedtop.png', 'delivery/macbook-v11w/p1-closedtop.png', '1 闭合俯视 closedtop', '900×724 · 白底', '俯视图.png'],
  ['p2-screenfront.png', 'delivery/macbook-v11w/p1-screenfront.png', '2 屏幕正面 screenfront', '1000×627 · 白底', '正视图.png'],
  ['p3-kb.png', 'delivery/macbook-v11w/p1-kb.png', '3 键盘+触控板 kb', '880×680 · 白底', '键盘和触控板.png'],
  ['p4-ports.png', 'delivery/macbook-v11d/p1-portstele.png', '4 左侧接口 portstele（长焦侧视）', '1180×395 · 暗底', 'official-mbp14-ports-1.jpg'],
  ['p5-hero.png', 'delivery/macbook-v11d/p1-hero.png', '5 3/4 开盖 hero', '860×520 · 暗底', 'official-mbp14-hero.jpg'],
  ['p6-bottom.png', 'delivery/macbook-v11g/p1-bottom.png', '6 底面 bottom', '880×600 · 灰底', 'apple-mbp13-bottom-case-official.jpg'],
...
const card = (x) => `
    <figure>
      <a href="./v11/${x.file}" target="_blank"><img src="./v11/${x.file}" alt="${x.title}" loading="lazy"></a>
...
<body>
  <h1>MacBook Pro 14″ · r12：键盘白线 / 前缘凹槽 / 端口补片</h1>
...
`
fs.writeFileSync(path.join(R, 'web/draw/v11.html'), html)
const missing = [...protocol, ...qa].filter((x) => !x.ok)
console.log(`gallery written: web/draw/v11.html（协议 ${protocol.length} + 特写 ${qa.length}${missing.length ? `，缺 ${missing.length} 张` : ''}）`)
