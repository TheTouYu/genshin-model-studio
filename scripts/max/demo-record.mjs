...
if (arg('quality', '')) await evalJS(`window.__photo.quality(${JSON.stringify(arg('quality', 'high'))})`)
if (arg('vbr', '')) await evalJS(`window.__vbr = ${Number(arg('vbr', '4500000'))}`)
if (arg('nocopy', '')) await evalJS(`window.__noCopy = true`)
await evalJS(src)
await sleep(300)
