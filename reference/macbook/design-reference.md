...
| HDMI | **14.98 mm** | 右侧**靠转轴**（⬅ 2026-09-09 修正，旧写「前部」方向反了） |
| SDXC | **27.21 mm** | 右侧**靠前缘**（⬅ 同上修正） |

- 所有开口**纵向中心线一致**（距侧壁上沿 3.79 mm，即位于 15.5 mm 闭合厚度的中部偏上）。
- **朝向修正（2026-09-09，端口高度 bug 修复时发现）**：本节旧写「左（前→后）MagSafe/USB-C/USB-C/3.5mm」，
  实际**方向相反** —— MagSafe 与 HDMI 靠**转轴端**，jack 与 SDXC 靠**前缘**。证据：用户参考图
  `底盖和100°侧面姿态.png`（上盖向左倾 → 转轴在左，MagSafe 紧邻转轴）+ `设计图.png` 左/右视图与
  「左侧接口/右侧接口」特写（脚垫出现在 MagSafe/HDMI 旁，脚垫距端 22.7 mm）+ 官方图 `ports-1/2.jpg`
  的圆角端即转轴端。实测 z（mm，机身中心为 0，前缘 +110.6）：左 −82.5/−64.2/−49.4/−36.6；
  右 −83.0/−64.7/−39.8。
- 开口高度受内腔阴影影响偏大 ⇒ 取行业标准值更可靠：USB-C 2.6 mm、SD 2.1 mm、3.5 mm 孔 Ø4.4 mm；
  **HDMI 4.5 mm**（= HDMI Type-A 插头高 4.45 mm；旧写 5.8 mm 是插头外框误用，实测暗区 28px/6.04 = 4.6 mm）。
- 散热槽：细长条（高 ≈0.4–1.5 mm），**两侧同 z 区间 +24..−20 mm（机身中部，非转轴端）**，
  与机身轮廓平行；官方图 `ports-1/2.jpg` 的 y=98 暗线即此槽。
...
1. **机身三围与 13" MBP 2020 几乎完全一致（差 ≤0.4 mm）** ⇒ 外形语言应按 **13" MBP 2020** 复刻：等厚侧壁、宽边框（上 13.6 / 下 11.9 / 左右 8.0 mm，**上厚下薄**）、Touch Bar 行、宽边距、R10 mm 圆角。
...
## 13. 机型候选（用户已裁决：暂不做 v3）

> ✅ **用户裁决（2026-09-09，本会话真人帧，原话「不做，等我给你指示」）**：v3 **暂不排期**，等用户后续指示。下方机型候选仅为研究侧建议，**不构成决策、不进入排期**。

> ⚠ 历史证据状态（裁决前）：本档案未获得任何用户机型选择。
...
## 14. 键帽细节档案（来源：`gen-04-键盘按键细节.png`，2026-09-09 用户补充）

> 生成图：2198×1231，md5 `1886ab94b504dd9b1164dfbecc973368`。**用途 = 结构层次参考**（键帽剖面、剪刀脚、字符层、长键平衡杆、双曲面），**不是尺寸权威**——尺寸冲突时一律以本档案官方实测为准。

### 14.1 生成图给出的键帽结构与工艺（🔵 采纳为结构依据）

| 项 | 值 |
|---|---|
| 材质 / 表面 | PBT / 微磨砂 |
| 键帽厚 | 1.20 mm（剖面键帽高 7.00 mm） |
| 字符工艺 | 激光镭雕 + 白色背光透光，字符层厚 0.10 mm |
| 颜色 | 深空灰（Pantone 426C，近似 `#25282A`） |
| 轮廓 | **双曲面键帽**：X 向 R300 / Y 向 R400 |
| 圆角 | 键帽顶面圆角 R1.00 mm |
| 位公差 | ±0.05 mm |
| 键帽顶面 | 14.00 × 14.00 mm |
| 键底结构 | 13.60 × 13.60 mm：剪刀脚安装位 + 稳定柱 Ø2.00 + 定位柱 + 透光孔 |
| 长键 | 空格 114.00、左 Shift 53.00、右 Shift 65.00、Return 48.00 × 29.00、Tab 28.00、Delete 28.00、Caps 41.00、方向键 14.00 × 14.00；底部 2–3 固定点 + **钢丝平衡杆卡扣** |
| 字符表 | 六行（功能键区 / 数字符号 / Q–P / A–L / Z–M / 底排 fn·control·option·command·space·command·option·方向键），逐键尺寸与字符齐全 |

### 14.2 与官方实测的冲突裁决（⚠ 建模必须按右列）

| 项 | 生成图 | 官方实测（§3.2 / §3.3） | 裁决 |
|---|---|---|---|
| 键距 | 未给（绘图间距 ≈16.8 mm） | **列 19.05 / 行 18.5 mm**（两路互证） | **官方** |
| 空格键宽 | 114.00 mm（= 标准 ANSI 6u） | **92.5 mm（5u）**，比标准 6.25u 短 25.5 mm | **官方** |
| 键帽顶面 | 14.00 × 14.00 | 未公布（❌ NOT FOUND） | 采纳生成图，**须与官方图目视比例 A/B 复核** |
| 剪刀脚 / 字符层 / 透光 / 双曲面 / 圆角 | 有 | 未公布 | **全部采纳** |

### 14.3 逐位置细节层次（本轮建模清单，数值出处见对应章节）

logo **37.1 × 45.6 mm（含叶）· 上偏 4.1 mm · 镜面嵌件**（§2）｜前缘开盖凹槽 **宽 ≈1/8 机宽 · 深 ≈1.5 mm**（§2）｜扬声器点阵竖带贯穿键盘高度、**多为装饰孔**（§2/§8）｜底盖铭牌 **≈79 × 14 mm · 三行 78/51/24 mm · 距后缘 11.5–25.2 mm**（§8）｜脚垫 **4 × Ø15.0 · 距左右 22.0 · 距前后 22.7 mm**（§8）｜屏幕黑边 **左右 5.1 mm · 上下合计 24.8 mm**（§5）｜刘海 **40 × 6 mm + 摄像头孔圈**（§5）｜转轴圆柱 + 铰链盖（§6）｜接口开口（§7.2）。
