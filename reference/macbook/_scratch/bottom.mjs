import {decodePNG} from './png.mjs';
const {w,h,ch,px}=decodePNG(process.argv[2]);
const at=(x,y)=>{const i=(y*w+x)*ch;return [px[i],px[i+1],px[i+2]];};
console.log('dims',w,h);
const cls=(x,y)=>{const [r,g,b]=at(x,y);const M=Math.max(r,g,b),N=Math.min(r,g,b);const sat=M===0?0:(M-N)/M;
  if(b>r+30&&b>g+10&&sat>0.25)return 'B'; if(M>244&&sat<0.05)return '.'; if(M<110)return '#'; return 'o';};
let bb=[1e9,1e9,-1,-1]; const cnt={};
for(let y=0;y<h;y++)for(let x=0;x<w;x++){const c=cls(x,y); cnt[c]=(cnt[c]||0)+1;
  if(c==='o'||c==='#'){if(x<bb[0])bb[0]=x;if(y<bb[1])bb[1]=y;if(x>bb[2])bb[2]=x;if(y>bb[3])bb[3]=y;}}
console.log('content bbox',bb,'w',bb[2]-bb[0]+1,'h',bb[3]-bb[1]+1,'aspect',((bb[2]-bb[0]+1)/(bb[3]-bb[1]+1)).toFixed(4));
console.log('class counts',cnt);
// find dark blobs (feet): grid 20px, mark cells with >30% dark
const GX=20; let rows=[];
for(let y=0;y<h;y+=GX){ let s=''; for(let x=0;x<w;x+=GX){ let d=0,n=0; for(let yy=y;yy<Math.min(y+GX,h);yy++)for(let xx=x;xx<Math.min(x+GX,w);xx++){n++; if(cls(xx,yy)==='#')d++;} 
  const f=d/n; s+= f>0.6?'#':f>0.25?'+':f>0.05?'.':' '; } rows.push(String(y).padStart(3)+' '+s); }
console.log(rows.join('\n'));
