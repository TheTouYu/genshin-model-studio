...
const SIDES=8;
const path7=[0.80,0.90,1.02,1.16,1.25,1.42,1.60].map(y=>[0,y,0]); // 7 control pts -> segs6 -> 7 rings
function build(path, armK, chestRx, shRx, armRootX, handY, armDepth){
  const profiles=[[0.125,0.085,0.100],[0.150,0.100,0.120],[0.120,0.082,0.082],[chestRx,0.130,0.100],[shRx,0.100,0.090],[0.085,0.085,0.098],[0.020,0.020,0.025]];
  ...
  function armRings(s){return [
    {c:[s*armRootX,1.245,0.001], dir:[s*1,-0.05,0], ru:0.045, rv:d, mix:0.32},
    {c:[s*(armRootX+0.05),1.235,0.001], dir:[s*1,-0.08,0], ru:0.048, rv:d-0.004, mix:0.80},
    {c:[s*(armRootX+0.065),1.06,0.000], dir:[s*0.28,-0.96,0], ru:0.040, rv:d-0.008, mix:1},
    ...(armK===4?[{c:[s*(armRootX+0.075),handY,0.012], dir:[s*0.05,-0.999,0], ru:0.020, rv:0.038, exp:0.8, mix:1}]:[])
  ];}
  const socketRings = path.length===8?[3,4]:[3,4]; // chest,shoulder
  ...
  return {v:mesh.vertices.length,t:mesh.faces.length/3,one:seam.onePiece,si:gate.checks.selfIntersections.intersectingPairs,ar:gate.checks.areaRatio.value,sk:gate.checks.skinny.count,skp:gate.checks.skinny.pct,nm:gate.checks.normals.invertedCount,deg:gate.checks.degenerate.count};
}
function row(l,o){console.log(l.padEnd(30),JSON.stringify(o));}
row('path7 K3 chest.15 sh.15 rx.20', build(path7,3,0.150,0.150,0.200,0.74,0.055));
row('path7 K3 chest.14 sh.14 rx.21', build(path7,3,0.140,0.140,0.210,0.74,0.052));
row('path7 K3 chest.15 sh.16 rx.20', build(path7,3,0.150,0.160,0.200,0.74,0.052));
row('path7 K4 chest.15 sh.15 rx.20', build(path7,4,0.150,0.150,0.200,0.72,0.055));
row('path7 K4 chest.14 sh.14 rx.21', build(path7,4,0.140,0.140,0.210,0.72,0.052));
