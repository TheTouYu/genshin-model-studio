...
const path=[0.80,0.90,1.02,1.15,1.24,1.30,1.44,1.60].map(y=>[0,y,0]);
const profiles=[[0.125,0.085,0.100],[0.150,0.100,0.120],[0.120,0.082,0.082],[0.150,0.130,0.100],[0.162,0.100,0.090],[0.050,0.050,0.048],[0.078,0.080,0.092],[0.020,0.020,0.025]];
...
function run(SIDES){
  ...
  const aR=extrudePatch(m,bv([3,4],right),armRings(1),{axis:[0,-1,0],color:SKIN,cap:true});
  const aL=extrudePatch(m,bv([3,4],left),armRings(-1),{axis:[0,-1,0],color:SKIN,cap:true});
  ...
  return {SIDES,v:m.vertices.length,t:m.faces.length/3,armB:aR.B,si:gate.checks.selfIntersections.intersectingPairs,nm:gate.checks.normals.invertedCount,one:seam.onePiece};
}
for(const s of [8,10,12,14,16]) console.log(JSON.stringify(run(s)));
